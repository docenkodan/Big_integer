...
big_integer::big_integer(const big_integer &other) :
		_digits(other._digits), _is_negative(other._is_negative) {
}
...
std::ostream& operator <<(std::ostream &os, const big_integer &bi) {
	if (bi._digits.empty())
		...
		for (long long i = static_cast<long long>(bi._digits.size()) - 2;
				i >= 0; --i)
...
bool operator ==(const big_integer &left, const big_integer &right) {
	...
	if (left._digits.empty()) {
		if (right._digits.empty()
				|| (right._digits.size() == 1 && right._digits[0] == 0))
			return true;
		else
			return false;
	}
...
const big_integer operator +(big_integer left, const big_integer &right) {
	...
	int carry = 0;
	for (size_t i = 0;
			i < std::max(left._digits.size(), right._digits.size())
					|| carry != 0; ++i) {
		if (i == left._digits.size())
			left._digits.push_back(0);
		left._digits[i] += carry
...
const big_integer operator *(const big_integer &left,
	...
	for (size_t i = 0; i < left._digits.size(); ++i) {
		...
		for (size_t j = 0; j < right._digits.size() || carry != 0; ++j) {
			long long cur = result._digits[i + j]
					+ left._digits[i] * 1LL
							* (j < right._digits.size() ? right._digits[j] : 0)
					+ carry;
...
std::string big_integer::toBinary() const {
	big_integer bi(*this);
	std::string result;
	while (bi._digits.size() >= 1 && bi._digits[0] >= 1) {
		result += bi.odd() ? '1' : '0';
		bi /= 2;
	}
